...
$(document).ready(function(){
	...
	var status = false; // 댓글과수정을 동시에 못하게 하기위한 상태값
	...
	$(document).on("click","button[name='reply_del']",function(){
        ...
        // 비밀번호 check
        if(check){
        	if(r_type == "main"){// depth 가 0이면 하위댓글 까지 삭제
        		// 삭제할때 하위댓글도 삭제
        		var prevTr = $(this).parent().parent().next(); // 다음댓글
        		
        		while(prevTr.attr("reply_type") == "sub"){// 댓글의 다임이 sub 면 계속넘어감
        			prevTr.remove();
	...
	$(document).on("click","button[name='reply_modify']",function(){
        ...
        // 상태값으로 수정과 대댓글 동시에 불가능하게 하기
        if(status){
        	alert("수정과 대댓글은 동시에 불가능 합니다");
        }
        if(check){
            status = true;
